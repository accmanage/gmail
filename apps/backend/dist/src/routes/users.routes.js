...
usersRouter.post("/", validate(z.object({ body: userBody.extend({ password: z.string().min(8) }) })), asyncHandler(async (req, res) => {
...
usersRouter.put("/:id", validate(z.object({ params: z.object({ id: z.coerce.number() }), body: userBody.partial() })), asyncHandler(async (req, res) => {
...
usersRouter.delete("/:id", validate(z.object({ params: z.object({ id: z.coerce.number() }) })), asyncHandler(async (req, res) => {
    const id = Number(req.params.id);
    if (req.user?.id === id)
