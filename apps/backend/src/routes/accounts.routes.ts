...
function safeAccount(account: any, includeSecrets = false) {
  return {
    id: account.id,
    email: account.email,
  ...
  asyncHandler(async (req, res) => {
    ...
    const accounts = await prisma.account.findMany({
      where: {
        ...
        status: status && status in AccountStatus ? (status as AccountStatus) : undefined
      },
      include: {
        pool: true,
...
accountsRouter.post(
  "/",
  validate(z.object({ body: accountBody })),
  asyncHandler(async (req, res) => {
...
accountsRouter.put(
  "/:id",
  validate(z.object({ params: z.object({ id: z.coerce.number() }), body: accountBody.partial() })),
  asyncHandler(async (req, res) => {
...
accountsRouter.post(
  "/import",
  validate(z.object({ body: z.object({ csv: z.string().min(1) }) })),
  asyncHandler(async (req, res) => {
    const parsed = Papa.parse<Record<string, string>>(req.body.csv, { header: true, skipEmptyLines: true });
    if (parsed.errors.length) throw new AppError(422, parsed.errors[0]?.message ?? "Invalid CSV");
    const imported = [];
