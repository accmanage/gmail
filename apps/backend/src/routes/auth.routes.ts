...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user.isActive) throw new AppError(403, "User account is suspended");
    if (user.validityUntil && user.validityUntil < new Date()) throw new AppError(403, "User access expired");
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!incoming) throw new AppError(401, "Refresh token required");
    ...
    if (!session || !session.user.isActive) throw new AppError(401, "Invalid refresh token");
  ...
  asyncHandler(async (req, res) => {
    ...
    if (bearerToken) {
      try {
        ...
        sessionIds.add(accessPayload.sessionId);
      } catch {
        // Best-effort logout should continue even with a stale access token.
      }
    }
    ...
    if (refreshToken) {
      try {
        ...
        sessionIds.add(refreshPayload.sessionId);
      } catch {
        // Cookie clearing still proceeds for expired or malformed refresh tokens.
      }
